...
class User extends CI_Controller
{
    ...
    public function create() 
    {
        ...
        // $this->db->select('id_users as kode', FALSE);
    ...
    public function create_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $level = $this->input->post('id_user_level');
            if($level != 1 OR $level != 2){
                $aturan = $this->aturan[0];
            }else{
                $aturan = $this->aturan[1];
            }
            $data = array(
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            ...
            $data['data_fak'] = $this->Fakultas_model->get_all();
            $data['data_prodi'] = $this->User_model->get_by_st($row->kode_fak)->result();
            $this->template->load('template','user/tbl_user_form', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('user'));
    ...
    public function word()
    {
        ...
        $data = array(
            'tbl_user' => $this->User_model->get_all(),
            'start' => 0
        );
    ...
    function prodi_json($id){
        $data['prodi'] = $this->User_model->get_by_fak($id)->result_array();
